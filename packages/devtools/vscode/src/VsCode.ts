          ...
          run(
            f(context).pipe(
              ...
              Effect.catchAllCause((_) => {
                return Effect.log("unhandled defect in webview view provider", _)
              }),
...
export const configWithDefault = <A>(
  ...
  setting: string,
  defaultValue: A
): Effect.Effect<ExtConfig.ConfigRef<A>, never, Scope.Scope> =>
  Effect.gen(function*() {
...
<R, E>(
  ...
  Effect.gen(function*() {
    ...
    if (provider.parent) {
      vscodeProvider.getParent = (element) =>
        Effect.runPromise(
          Effect.map(provider.parent!(element), Option.getOrUndefined)
        )
    }
    if (provider.resolve) {
      vscodeProvider.resolveTreeItem = (item, element, token) =>
        runWithTokenDefault(
          Effect.map(
...
export const launch = <E>(layer: Layer.Layer<never, E, VsCodeContext>) =>
  ...
  }).pipe(Effect.catchAllCause((e) => {
    return Effect.logFatal(e)
  }))
...
export const revealFile = (
  ...
  thenable(() => vscode.workspace.openTextDocument(vscodeUriFromPath(path))).pipe(
    Effect.flatMap((doc) =>
      thenable(() =>
        selection ? vscode.window.showTextDocument(doc, { selection }) : vscode.window.showTextDocument(doc)
      )
    )
  )
