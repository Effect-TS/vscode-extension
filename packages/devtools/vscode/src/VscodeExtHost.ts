...
export const layer = Layer.unwrapScoped(Effect.gen(function*() {
  ...
  return Layer.mergeAll(
    Layer.succeed(ExtCommand.ExtCommandHostCapability, {
      registerCommand,
      executeCommand
    }),
    Layer.succeed(ExtTreeView.ExtTreeViewHostCapability, {
      registerTreeView,
    ...
    Layer.succeed(ExtWebView.ExtWebViewHostCapability, {
      registerWebView
    }),
    Layer.succeed(ExtConfig.ExtConfigHostCapability, {
      readConfig,
      registerConfig: () => Effect.void
    }),
    Layer.succeed(ExtWorkspace.ExtWorkspaceHostCapability, {
      asWorkspaceRelativePath
    }),
    Layer.succeed(ExtTextEditor.ExtTextEditorHostCapability, {
      revealFileLineColumnRange
    }),
    Layer.succeed(ExtWhenClause.ExtWhenClauseHostCapability, {
      setVariable
    })
  )
}))
