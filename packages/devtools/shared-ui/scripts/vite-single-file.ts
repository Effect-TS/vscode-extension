...
export function replaceScript(
  ...
  const inlined = html.replace(
    reScript,
    (_, beforeSrc, afterSrc) => `<script${beforeSrc}${afterSrc}>${newCode.trim()}</script>`
  )
...
export function replaceCss(html: string, scriptFilename: string, scriptCode: string): string {
  ...
  const inlined = html.replace(
    ...
    (_, beforeSrc, afterSrc) =>
      `<link${beforeSrc} id="vscode-codicon-stylesheet" href="data:text/css;charset=utf-8;base64,${
        Buffer.from(newCode).toString("base64")
      }" ${afterSrc} />`
  )
  return inlined
}
...
export function viteSingleFile({
  ...
  return {
    ...
    generateBundle(_, bundle) {
      ...
      const bundlesToDelete = [] as Array<string>
