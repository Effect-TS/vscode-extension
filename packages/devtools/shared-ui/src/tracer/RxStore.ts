import { Atom, Result as _Result } from "@effect-atom/atom-react"
import type * as DevToolsDomain from "@effect/experimental/DevTools/Domain"
import { Clock, Schema } from "effect"
import * as Effect from "effect/Effect"
import * as Stream from "effect/Stream"
import { isString } from "effect/String"
import * as SubscriptionRef from "effect/SubscriptionRef"
import * as WebviewMessaging from "../WebviewMessaging"
import type { TraceEvent } from "./components/TraceViewerUtils"
import { Booted, HostMessage } from "./messages.ts"
...
export class RxStore extends Effect.Service<RxStore>()("RxStore", {
  accessors: true,
  dependencies: [WebviewMessaging.layer(Schema.encodeUnknownSync(Booted)(new Booted()))],
  scoped: Effect.gen(function*() {
    const { events } = yield* WebviewMessaging.WebviewMessaging
    ...
    yield* Stream.fromPubSub(events).pipe(
      Stream.filterMap(Schema.decodeUnknownOption(HostMessage)),
      Stream.tap((message) => {
        switch (message._tag) {
          case "ExternalSpan": {
...
const runtime = Atom.runtime(RxStore.Default).pipe(Atom.keepAlive)

export const traceEventsInternal = runtime.subscribable(
  Effect.map(RxStore, (service) => service.traceEvents)
)

export const traceEventsRx = Atom.make((get) => {
