...
const drawTraceBar = (
  ...
  if (textWidth < availableWidth) {
    ...
    while (
      ctx.measureText(truncatedText + ellipsis).width > availableWidth &&
...
const TraceViewer: React.FC<TraceViewerProps> = ({
  ...
  const getTraceAtPosition = useCallback(
    (x: number, y: number): TraceEvent | null => {
      ...
      for (const trace of traces) {
        ...
        const traceY = timelineHeight +
          trace.depth * (barHeight + barPadding) +
          viewState.offsetY
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      const newLeftDuration = BigInt(
        Math.round(Number(leftDuration) * zoomFactor)
      )
      const newRightDuration = BigInt(
        Math.round(Number(rightDuration) * zoomFactor)
      )
      ...
      const maxDepth = traces.reduce(
        (max, trace) => Math.max(max, trace.depth),
        0
      )
