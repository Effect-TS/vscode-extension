...
const TraceMinimap: React.FC<MinimapProps> = ({
  ...
  const { barHeight = 30, barPadding = 4, minimapHeight = 80 } = options
  ...
  const draw = useCallback(() => {
    ...
    const minimapBarHeight = Math.min(
      Math.max(1, minimapHeight / maxDepth),
      minimapHeight / 10
    )
    ...
    traces.forEach((trace) => {
      ...
      const width = Math.max(
        1,
        Number(trace.endTime - trace.startTime) * pixelsPerNano
      )
