...
const TraceInfoPanel: React.FC<TraceInfoPanelProps> = ({
  onGoToLocation,
  timeOrigin,
  trace
}) => {
  ...
  return (
    <div style={panelStyle}>
      <h3 style={{ marginTop: 0, marginBottom: styles.headerMarginBottom }}>
        Span Details
      </h3>
      ...
      <div style={fieldStyle}>
        ...
        <div
          style={{
            ...
            fontSize: styles.monospaceFontSize
          }}
        >
          {trace.spanId}
        </div>
      ...
      {trace._tag === "Span" ?
        (
          <div style={fieldStyle}>
            ...
            <div style={valueStyle}>
              {formatDuration(trace.status.startTime - timeOrigin)}
            </div>
          </div>
        ) :
        null}
      ...
      {trace._tag === "Span" && trace.status._tag === "Ended" ?
        (
          <div style={fieldStyle}>
            ...
            <div style={valueStyle}>
              {formatDuration(trace.status.endTime - timeOrigin)}
            </div>
          </div>
        ) :
        null}
      ...
      {trace._tag === "Span"
        ? Array.from(trace.attributes.entries()).map(([name, value]) => (
          <div style={fieldStyle} key={name}>
            <strong>
              {name === "@effect/devtools/trace" ? "Location" : String(name)}
            </strong>
            <div style={valueStyle}>
              {name === "@effect/devtools/trace" ?
                (
                  <a href="#" onClick={onGoToLocation}>
                    Go to location
                  </a>
                ) :
                (
