...
import * as ExtTreeView from "../core/ExtTreeView.js"
...
export const layer = Layer.unwrapScoped(Effect.gen(function*() {
  const extCommandHost = yield* ExtCommand.ExtCommandHostCapability
  const extWebViewHost = yield* ExtWebView.ExtWebViewHostCapability
  ...
  const whenEvaluator = yield* ExtWhenClause.ExtWhenEvaluator
              ...
              switch (_._tag) {
                ...
                case "ExecuteTitleAction": {
                  return yield* pipe(
                    ...
                    Effect.flatMap((command) => extCommandHost.executeCommand(command, undefined))
                  )
                }
                  ...
                  return yield* pipe(
                    ...
                    Effect.flatMap((item) =>
                      pipe(
                        ...
                        Effect.flatMap((action) => extCommandHost.executeCommand(action.command, item))
                      )
                    )
                  )
                }
  ...
  return Layer.succeed(
    ExtTreeView.ExtTreeViewHostCapability,
    ExtTreeView.ExtTreeViewHostCapability.of({
      registerTreeView: (treeView, builder) => {
        const [webView, webBuilder] = toWebview(treeView, builder)

        return extWebViewHost.registerWebView(webView, webBuilder)
      },
